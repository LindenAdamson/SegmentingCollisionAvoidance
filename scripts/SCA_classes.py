...
class Segmentation_Collision_Avoidance:
    ...
    def plot(self):
        ...
        for color_idx, obj in enumerate(obj_list):
            ...
            axs[0].plot(outline[:,0],outline[:,1],color=plt_colors[color_idx % plt_colors.__len__()], linewidth=3)
        axs[0].set_xlim(0,cols)
        axs[0].set_ylim(rows+1,-1)
        ...
        for color_idx, obj in enumerate(window.objects_in_scope):
            ...
            axs[1].plot(box[:,0], box[:,1], color=plt_colors[color_idx % plt_colors.__len__()])
        for i, a in enumerate(angle_steps):
            if a == 0:
                plot_arc(0, arcs[i], axs[1])
                if a == window.best_steering_angle:
                    for_legend = plot_arc(0, arcs[i], axs[1], True)
            else:
                plot_arc(window.make_angle_path_circle(a)[0], arcs[i], axs[1])
                if a == window.best_steering_angle:
                    for_legend = plot_arc(window.make_angle_path_circle(a)[0], arcs[i], axs[1], True)
        width, height = axs[1].get_figure().get_size_inches()
        xdim = 6
        axs[1].set_xlim(-xdim,xdim)
        ydim = xdim / height * width
        axs[1].set_ylim(0,2 * ydim)
        axs[1].legend(handles=[for_legend], loc='upper center', prop={'size': 15})
        plt.tight_layout()
        return fig
...
class Frame:
    ...
    def filter_objects(self):
        ...
        self.low_confidence = np.where(np.logical_or(self.depthImg > Config.get("magic_trust_number"),\
            self.depthImg == 0), 1, 0)
